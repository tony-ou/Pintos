...
typedef struct wrapper_execute{
  ...
  struct dir *working_dir;                     /* Working dir.*/
} wrapper_execute;
...
process_execute (const char *file_name)
{
  ...
  /* Make a copy of working directory of parent */ 
  struct thread* cur = thread_current();
  struct dir *working_dir = cur->working_dir;
  if (working_dir == NULL)
    aux.working_dir = dir_open_root();
  else
    aux.working_dir = dir_reopen(working_dir);

  if (aux.working_dir == NULL)
    return TID_ERROR;
...
start_process (void *aux)
{
  ...
  else
  {
    ...
    cur->working_dir = exec->working_dir;
...
load (const char *file_name, void (**eip) (void), void **esp)
{
  ...
  file = file_open(filesys_open (exec_name));
  if (file == NULL)
    {
